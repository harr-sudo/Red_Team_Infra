...
Upload files to the attack box via SCP through the Dashboard Server jump.
"""
...
def connection_info():
    """Get connection details (Dashboard Server jump host, attack box IP, SSH key) for a project."""
    try:
        ...
        # GOAD jumpbox is the only deployment-local SSH hop that still exists.
        jumpbox_ip = outputs.get("goad_jumpbox_public_ip", {}).get("value")

        # Determine hop host: prefer the Dashboard Server jump (its EIP — the
        # sole SSH jump now that the per-deployment bastion is gone), fall
        # back to the GOAD jumpbox when no dashboard EIP is resolvable.
        dashboard_eip = dashboard_settings_service.resolve_eip()["effective"]
        hop_ip = dashboard_eip or jumpbox_ip
        hop_type = "dashboard" if dashboard_eip else ("jumpbox" if jumpbox_ip else None)
