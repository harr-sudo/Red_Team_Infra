...
def _resolve_jumpbox_ip(state: dict) -> str | None:
    """Pull the jumpbox or bastion public IP from the state.json outputs.

    For c2-* deployments the bastion is the SSH pivot; for combined we
    prefer goad_jumpbox_public_ip. We try both in order.
    """
    outputs = state.get("output") or {}
    for key in (
...
def provision(project: str):
    ...
    if not jumpbox_ip:
        return jsonify(
            {
                "success": False,
                "error": "Could not resolve jumpbox / bastion IP from deployment outputs",
            }
        ), 500
