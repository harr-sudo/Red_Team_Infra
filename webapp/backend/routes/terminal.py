...
def _direct_ssh_allowlist():
    ...
    Secure default (H3 fix): when the env var is unset/empty we return an
    EMPTY list, meaning no host is eligible for a direct connection and every
    session is routed through the Dashboard Server jump. Operators opt specific
    public prefixes into direct SSH by setting the env var to a comma-separated
    list (e.g. `203.0.113.0/24,198.51.100.0/24`).
    """
    raw = (os.environ.get("DIRECT_SSH_ALLOWLIST_CIDRS") or "").strip()
    nets = []
    for tok in raw.split(","):
        ...
        try:
            ...
            try:
                ...
                pass
    # Secure default (H3 fix): when unset/empty, return an EMPTY allow-list so no
    # host is eligible for a direct connection and every session routes via the
    # Dashboard Server jump. Operators opt specific public prefixes into direct
    # SSH by setting DIRECT_SSH_ALLOWLIST_CIDRS explicitly.
    return nets
...
def _is_host_reachable(host, port=22, timeout=1.5):
    """Decide if `host` should be SSHed to directly (skip Dashboard Server jump).
    ...
    but that the operator wanted to reach via the Dashboard Server jump.
    Two tightenings:

    1. The host's IP must fall inside an operator-supplied allow-list of
       CIDR prefixes (env: `DIRECT_SSH_ALLOWLIST_CIDRS`). The default is now
       EMPTY (H3 fix) — with no allow-list, nothing is eligible for direct
       SSH and every session routes via the Dashboard Server jump.
    ...
    the Dashboard Server jump. We log a clear "Falling back to Dashboard
    Server jump" line so operators can debug routing decisions.
    """
    global _DIRECT_SSH_DEFAULT_WARNED

    nets = _direct_ssh_allowlist()
    if not nets:
        # Secure default (H3 fix): no allow-list configured → never connect
        # directly; always route via the Dashboard Server jump.
        if not _DIRECT_SSH_DEFAULT_WARNED:
            _DIRECT_SSH_DEFAULT_WARNED = True
            try:
                print(
                    "[terminal] DIRECT_SSH_ALLOWLIST_CIDRS unset — routing all "
                    "sessions via the Dashboard Server jump (secure default). "
                    "Set DIRECT_SSH_ALLOWLIST_CIDRS to opt specific public "
                    "prefixes into direct SSH."
                )
            except Exception:
                pass
        return False
        ...
        if not any(host_ip in n for n in nets):
            try:
                print(
                    ...
                    "— falling back to Dashboard Server jump."
                )
            except Exception:
                pass
            return False
    ...
    try:
        ...
        try:
            print(
                ...
                f"{timeout}s — falling back to Dashboard Server jump."
            )
        except Exception:
            pass
        return False
...
def terminal_ssh(ws):
    """SSH session to a remote instance, optionally via the Dashboard Server jump."""
    if len(_active_sessions) >= _MAX_SESSIONS:
        ws.send('\r\n\x1b[31mSession limit reached (max 5). Close a tab first.\x1b[0m\r\n')
        ws.close()
        return
    ...
    # Dashboard Server jump EIP. Prefer the new `jump` param; fall back to
    # the legacy `bastion` param for older frontends still in flight.
    jump = params.get('jump') or params.get('bastion')
    ...
    using_jump = False
    if jump and not _is_host_reachable(host):
        cmd += ['-J', f'ubuntu@{jump}']
        using_jump = True
    elif jump and _is_host_reachable(host):
        ws.send('\x1b[36mDirect route available — skipping Dashboard Server jump\x1b[0m\r\n')
    ...
    ws.send(f'\x1b[36mConnecting to {user}@{host}' +
            (f' via Dashboard Server jump ({jump})' if using_jump else '') +
            '...\x1b[0m\r\n')
    ...
    audit_service.write(
        ...
        "terminal.start",
        target=f"{user}@{host}",
        details={"kind": "ssh", "via_jump": using_jump},
    )
