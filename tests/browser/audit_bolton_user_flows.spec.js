...
test('12. DEPLOYMENT SWITCH — switching from c2_adhoc → demo refreshes host list', async ({ page }) => {
    ...
    await page.waitForFunction(async () => {
        ...
        const b = await r.json();
        return b.deployments && b.deployments.some(d => d._filename === 'c2_adhoc_dev_harriss_macbook_pro_01');
    });
    await page.waitForTimeout(800);
    // Set c2_adhoc first
    await page.evaluate(() => {
        window.APP.activeDeployment.set('c2_adhoc_dev_harriss_macbook_pro_01');
...
test('15. C2-ONLY — c2-adhoc without enable_test_lab hides Bolt-ons sub-pill', async ({ page }) => {
    ...
    await page.waitForFunction(() => window.APP.activeDeployment.current === 'c2_adhoc_dev_harriss_macbook_pro_01');
    const subPills = await page.evaluate(() =>
        window.APP.computeVisibleSubPills(window.APP.activeDeployment),
    );
