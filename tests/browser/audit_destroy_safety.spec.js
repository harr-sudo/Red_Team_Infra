 ...
 *
 * NOTE: project goad_mini_dev_harriss_macbook_pro is known (per project
...
const TARGET_PROJECT = 'goad_mini_dev_harriss_macbook_pro';
...
test('Destroy preflight blocks when foreign modules present (no real destroy)', async ({ page }) => {
    ...
    await page.evaluate((payload) => {
        if (window.APP?.manage?._handleForeignModulesError) {
            ...
            window.APP.manage._handleForeignModulesError('goad_mini_dev_harriss_macbook_pro', payload);
        }
    }, synthetic409);
    await page.waitForTimeout(500);
    expect(confirmFired,
