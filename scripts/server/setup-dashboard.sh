...
# --- Singleton guard (AWS-tag backstop) ---------------------------------------
# The resume check earlier is state-based (terraform output). Run from a fresh
# clone with no local/remote state and it would miss an existing dashboard and
# provision a duplicate. The Dashboard Server always carries the fixed Name tag
# "redteam-dashboard-server" (terraform/main.tf pins project_name =
# "redteam-dashboard" for module.dashboard_server), so a single describe-instances
# call confirms whether a dashboard already lives in this region. Only one
# dashboard per region is supported.
EXISTING_DASH=$(aws ec2 describe-instances --region "$AWS_REGION" --filters "Name=tag:Name,Values=redteam-dashboard-server" "Name=instance-state-name,Values=pending,running,stopping,stopped" --query 'Reservations[].Instances[].[InstanceId,PublicIpAddress]' --output text 2>/dev/null || true)
if [ -n "$EXISTING_DASH" ]; then
    warn "A Dashboard Server already exists in $AWS_REGION (only one per region is supported):"
    echo "$EXISTING_DASH" | while read -r _did _dip; do
        warn "  instance $_did   public IP: ${_dip:-<stopped/none>}"
    done
    warn "Not provisioning a second one. To manage or repair the existing dashboard instead:"
    warn "  ./scripts/server/setup-dashboard.sh             # resume (rsync code + restart service)"
    warn "  ./scripts/server/setup-dashboard.sh --update-ip # restore SSH access if your IP changed"
    error "Existing dashboard detected via AWS tag — aborting to avoid a duplicate."
fi
