...
variable "management_cidr_blocks" {
  description = "CIDR blocks allowed for break-glass SSH/management access (your IP). Leave empty to reach instances only via the Dashboard Server jump."
  type        = list(string)
  default     = []

  validation {
    condition     = !contains(var.management_cidr_blocks, "0.0.0.0/0") && !contains(var.management_cidr_blocks, "::/0")
    error_message = "management_cidr_blocks must not contain 0.0.0.0/0 or ::/0 — that would expose break-glass SSH (and on GOAD, RDP 3389 / WinRM 5985-5986) to the entire internet. Use your operator IP(s), e.g. [\"203.0.113.10/32\"], or leave it empty to use the Dashboard Server jump only."
  }
}
