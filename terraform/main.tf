...
#   2. GOAD-Only: Training lab with a dedicated CS team server in the GOAD VPC (goad-*)
...
# AWS provider for the CREST Community AMI source region (eu-west-2 / London).
# Used READ-ONLY by the ccrts_lab module to look up the latest CREST AMI IDs.
# aws_ami_copy then duplicates the underlying snapshot into the deploy region;
# no EC2 / VPC / networking is ever provisioned in eu-west-2.
provider "aws" {
  alias  = "crest_source"
  region = "eu-west-2"
}
...
locals {
  ...
  install_goad_teamserver = local.is_goad_only # GOAD-only: dedicated CS team server on the .40 GOAD VPC host (NOT the jumpbox)
...
module "goad" {
  ...
  # CS Team Server on a dedicated .40 host inside the GOAD VPC (separate from the jumpbox), for GOAD-only mode
  install_cobalt_strike  = local.install_goad_teamserver
  cobalt_strike_s3_path  = var.cobalt_strike_archive_s3_path
  cs_teamserver_password = var.cs_teamserver_password
...
# =============================================================================
# CCRTS LAB MODULE — CREST exam-mirror lab
# =============================================================================
# Provisions the CCRTS lab in eu-central-1. CREST AMIs (owner 126620636130)
# are copied from eu-west-2 via aws_ami_copy + the aws.crest_source provider.
...
module "ccrts_lab" {
  ...
  crest_kali_ami_override    = var.crest_kali_ami_override
  crest_windows_ami_override = var.crest_windows_ami_override

  # Credentials (lab posture — weak by design)
  dc_admin_password = var.ccrts_dc_admin_password
  low_priv_password = var.ccrts_low_priv_password
...
module "dashboard_server" {
  ...
  # Tag the dashboard as its own shared control plane — NOT the deployment whose
  # tfvars happened to supply the root variables. DeploymentType is set here to
  # override the provider default_tags (which would otherwise stamp it with the
  # deployment's type), keeping cost attribution and semantics correct.
  tags = {
    Project        = "redteam-dashboard"
    DeploymentType = "dashboard"
    Environment    = "shared"
    ManagedBy      = "Terraform"
  }
}
