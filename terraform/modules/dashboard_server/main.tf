...
resource "aws_iam_policy" "dashboard" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      {
        # HIGH-2 fix: iam:PassRole is split out and constrained so the dashboard
        # can only hand roles to the services this project actually provisions
        # (EC2 instance profiles + the DLM snapshot-lifecycle role). Without the
        # iam:PassedToService guard, PassRole on Resource="*" is a textbook
        # privilege-escalation primitive (create an admin role, then pass it on).
        ...
        Condition = {
          StringEquals = {
            "iam:PassedToService" = ["ec2.amazonaws.com", "dlm.amazonaws.com"]
          }
        }
      },
      {
...
resource "aws_instance" "dashboard" {
  ...
  # Enforce IMDSv2 (HIGH-1): the dashboard holds a near-admin instance role and
  # is the only public-EIP instance, making it the highest-value metadata target.
  # Requiring a session token stops an SSRF/RCE on the Flask app from reading the
  # role's STS credentials over IMDSv1. (hop_limit 2 matches every other instance
  # module; 1 would be marginally tighter since nothing is containerised here.)
  metadata_options {
