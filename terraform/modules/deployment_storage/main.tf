...
data "aws_iam_policy_document" "bucket_policy" {
  ...
  statement {
    ...
    # CRITICAL-2 fix: the two conditions in a single Deny are AND-ed. The old
    # pairing (SourceVpc-not-allowed AND account-not-ours) meant a *same-account*
    # caller from outside the allowed VPCs was never denied here — so the headline
    # VPC confinement was a no-op at the bucket layer. We instead pair the VPC
    # check with a "SourceVpc must be present" guard: this statement now enforces
    # the VPC restriction for every VPC-originated request regardless of account,
    # while leaving non-VPC requests (Terraform from the operator workstation/CI,
    # which carry no SourceVpc) to be governed solely by the same-account check in
    # Statement 2 below.
    condition {
      test     = "Null"
      variable = "aws:SourceVpc"
...
# Scoped SSM access for the documented remote-management workflow
# (`aws ssm send-command` / `start-session`). CRITICAL-1 fix: replaces the broad
# AWS-managed AmazonSSMManagedInstanceCore policy — which carried S3 read grants
# and, having no aws:SourceVpc condition, undercut the S3 confused-deputy design
# built above. This grants ONLY the Session Manager + Run Command agent channels
# (no S3); those channel actions are not resource-scopable, so Resource = "*" is
# expected and is the same scope AWS's own managed policy uses for them.
locals {
  ssm_agent_actions = [
...
resource "aws_iam_role_policy" "ssm_c2_access" {
  count = local.create_c2_role ? 1 : 0
  name  = "${local.sanitized_name}-ssm-access-c2-policy"
  role  = aws_iam_role.cs_download_c2[0].id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      ...
      Effect   = "Allow"
      Action   = local.ssm_agent_actions
      Resource = "*"
    }]
  })
}

resource "aws_iam_role_policy" "ssm_goad_access" {
  count = local.create_goad_role ? 1 : 0
  name  = "${local.sanitized_name}-ssm-access-goad-policy"
